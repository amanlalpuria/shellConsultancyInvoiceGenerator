...
class InvoiceForm extends React.Component {
  ...
  handleCalculateTotal() {
    ...
    this.setState({
      ...
      cgsttaxAmount: (subTotal * (this.state.cgst / 100)).toFixed(2),
      sgsttaxAmount: (subTotal * (this.state.sgst / 100)).toFixed(2),
      discountAmmount: (subTotal * (this.state.discountRate / 100)).toFixed(2),
      total: (subTotal - this.state.discountAmmount + parseFloat(this.state.cgsttaxAmount) + parseFloat(this.state.sgsttaxAmount)).toFixed(2)
    });
  }
  ...
  /*handleCalculateTotal() {
    ...
    items.map(function(items) {
      var i =1;
      console.log("contains ", i , items.price);

      subTotal = parseFloat(subTotal + (parseFloat(items.price).toFixed(2))).toFixed(2);
    ...
    this.setState({
      ...
      this.setState({
        cgsttaxAmount: parseFloat(parseFloat(subTotal) * (this.state.cgst / 100)).toFixed(2)
      }, () => {
        this.setState({
          sgsttaxAmount: parseFloat(parseFloat(subTotal) * (this.state.sgst / 100)).toFixed(2)
        }, () => {
        this.setState({
      ...
      <Row>
          ...
          <Card className="p-4 p-xl-5 my-3 my-xl-4">
            ...
            <Row className="mb-5">
              <Col>
                ...
                <Form.Control placeholder={"Contact"} value={this.state.billToContact} type="text" name="billToContact" className="my-2" autoComplete="address" onChange={(event) => this.editField(event)} required="required"/>
                <Form.Control placeholder={"GST No:"} value={this.state.billToGST} type="text" name="billToGST" className="my-2" autoComplete="gst" onChange={(event) => this.editField(event)} required="required"/>
              </Col>
              {/*<Col>
        ...
        <Col md={4} lg={3}>
          <div className="sticky-top pt-md-3 pt-xl-4">
            ...
            <InvoiceModal showModal={this.state.isOpen} closeModal={this.closeModal} info={this.state} items={this.state.items} currency={this.state.currency} subTotal={this.state.subTotal} cgsttaxAmount={this.state.cgsttaxAmount} sgsttaxAmount={this.state.sgsttaxAmount} discountAmmount={this.state.discountAmmount} total={this.state.total}/>
